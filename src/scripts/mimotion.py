...
name = "小米运动"
header = {"User-Agent": "Mozilla/5.0 (Linux; Android 12; M2012K11AC Build/RKQ1.200826.002; wv) AppleWebKit/537.36 "
                        "(KHTML, like Gecko) Version/4.0 Chrome/90.0.4430.210 Mobile Safari/537.36 oppostore/201202 "
                        "MIUI/V125 brand/Redmi model/M2012K11AC"}
...
def main(param):
    ...
    for account in param:
        try:
            ...
            try:
                min_step = int(account.get("min_step", 10000))
            except Exception as e:
                print("初始化步数失败: 已将最小值设置为 10000", e)
                min_step = 10000
            try:
                max_step = int(account.get("max_step", 19999))
            except Exception as e:
                print("初始化步数失败: 已将最大值设置为 19999", e)
                max_step = 19999
            step = str(random.randint(min_step, max_step))
            login_token, userid = login(phone, password)
            if login_token == 0:
                msg = [
                    {"name": "帐号信息", "value": f"{phone[:3]}****{phone[-4:]}"},
...
if __name__ == "__main__":
    try:
        ...
        data = config.read_param('MIMOTION', 0)
    except Exception as e:
        import os, re, json
